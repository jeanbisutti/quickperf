...
/**
 * Reflection-based bridge so JUnit 4 runners (the plain
 * {@link QuickPerfJUnitRunner} and the Spring-aware
 * {@code QuickPerfSpringRunner}) can claim the current Surefire pool
 * thread as a "test thread" in {@code SqlRecorderRegistry} and
 * {@code ConnectionListenerRegistry} BEFORE any SQL is fired by the
 * test setup (e.g. Spring {@code ApplicationContext} startup, Hibernate
 * {@code EntityManagerFactory} creation, schema generation, ...).
 * <p>
 ...
 * running concurrently under Surefire's {@code parallel=all} mode.
 * <p>
 * Reflection is used so this {@code junit4-runner} module does not
 * gain a hard dependency on {@code sql-annotations}: the runner is
 ...
 */
public final class SqlTestThreadMarker {
     ...
     */
    public static void markCurrentThreadAsSqlTestThread() {
