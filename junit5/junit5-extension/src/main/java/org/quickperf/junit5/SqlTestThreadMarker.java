...
/**
 * Reflection-based bridge so the JUnit 5 extension
 * ({@link QuickPerfTestExtension}) can claim the current Surefire pool
 * thread as a "test thread" in {@code SqlRecorderRegistry} and
 * {@code ConnectionListenerRegistry} BEFORE any {@code @BeforeAll} /
 * {@code @BeforeEach} method runs (e.g. Spring {@code ApplicationContext}
 ...
 * running concurrently under JUnit 5 {@code parallel=concurrent} or
 * Surefire {@code parallel=all} mode.
 * <p>
 * Reflection is used so this {@code junit5-extension} module does not
 * gain a hard dependency on {@code sql-annotations}: the extension is
