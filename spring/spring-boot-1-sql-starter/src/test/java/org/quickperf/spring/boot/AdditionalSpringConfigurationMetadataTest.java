 ...
 *
 * <p>This test is duplicated from {@code quick-perf-springboot2-sql-starter}'s
 * {@code AdditionalSpringConfigurationMetadataTest}. The metadata JSON files in the two starter modules
 * must remain byte-identical; both tests share the same logic so accidental drift between the two files
 * is caught in either module independently.
 *
 * <p>This test is intentionally written using only JDK 1.7-compatible APIs (no Jackson, no JUnit 5)
 * because this module targets JDK 1.7 source/target compatibility.
 */
