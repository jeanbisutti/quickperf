...
@RestController
public class AsyncPlayerController {

    @Autowired
    private AsyncPlayerService asyncPlayerService;

    @GetMapping("/async/players")
    public List<PlayerWithTeamName> findAllAsync() throws Exception {
        return asyncPlayerService.findPlayersWithTeamNameAsync().get();
    }
